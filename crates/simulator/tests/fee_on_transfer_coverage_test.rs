...
fn addr_to_u256(a: Address) -> U256 { U256::from_be_slice(a.as_slice()) }
fn addr_hex(a: Address) -> String { format!("0x{}", a.iter().map(|b| format!("{:02x}", b)).collect::<String>()) }
fn u256_hex(v: U256) -> String { format!("0x{}", v.to_be_bytes::<32>().iter().map(|b| format!("{:02x}", b)).collect::<String>()) }

static PORT_COUNTER: std::sync::atomic::AtomicU16 = std::sync::atomic::AtomicU16::new(0);

struct AnvilGuard { child: Child, url: String }
...
impl AnvilGuard {
    ...
    fn rpc(&self, method: &str, params: &[&str]) {
        let output = Command::new("cast").args(["rpc", "--rpc-url", &self.url, method]).args(params)
            .output().expect("cast rpc");
        assert!(output.status.success(), "cast rpc {method}: {}", String::from_utf8_lossy(&output.stderr));
    }
    fn set_code(&self, addr: Address, bytecode: &str) {
        self.rpc("anvil_setCode", &[&format!("\"{}\"", addr_hex(addr)), &format!("\"{}\"", bytecode)]);
    }
    fn set_storage(&self, addr: Address, slot: U256, value: U256) {
        self.rpc("anvil_setStorageAt", &[&format!("\"{}\"", addr_hex(addr)), &format!("\"{}\"", u256_hex(slot)), &format!("\"{}\"", u256_hex(value))]);
    }
    async fn create_fork_state_warm(&self, addrs: &[Address]) -> RpcForkedState {
        ...
        let mut state = RpcForkedState::new_at_latest(provider.clone(), latest, 4_000_000_000, 1_000_000_000).expect("fork state");
        for addr in addrs {
            ...
            let nonce = provider.get_transaction_count(*addr).await.expect("get nonce");
            state.db.cache.contracts.insert(code_hash, code_bytecode);
            state.db.insert_account_info(*addr, AccountInfo { balance, nonce, code_hash, code: None, ..Default::default() });
        }
        state
    }
...
fn round_trip_clean_for_fee_only_loss() {
    ...
    assert!(matches!(classify_round_trip(base_in, base_out, 30, true, 200), RoundTripVerdict::Clean { .. }));
}
#[test]
fn round_trip_fee_on_transfer() {
    assert!(matches!(classify_round_trip(U256::from(1_000_000u64), U256::from(800_000u64), 30, true, 100), RoundTripVerdict::FeeOnTransfer { .. }));
}
#[test]
fn round_trip_honeypot_sell_failed() {
    assert!(matches!(classify_round_trip(U256::from(1_000_000u64), U256::ZERO, 30, false, 100), RoundTripVerdict::Honeypot { .. }));
}
#[test]
fn round_trip_honeypot_zero_recovery() {
    assert!(matches!(classify_round_trip(U256::from(1_000_000u64), U256::ZERO, 30, true, 100), RoundTripVerdict::Honeypot { .. }));
}
#[test]
fn round_trip_inconclusive_zero_base_in() {
    assert!(matches!(classify_round_trip(U256::ZERO, U256::from(1u64), 30, true, 100), RoundTripVerdict::Inconclusive { .. }));
}
#[test]
...
fn encode_v2_swap_lengths() {
    let d = encode_v2_swap(U256::from(u128::MAX), U256::from(u128::MAX), Address::repeat_byte(0xFF));
    assert_eq!(d.len(), 4 + 5 * 32);
    assert_eq!(&d[0..4], &[0x02, 0x2c, 0x0d, 0x9f]);
}
#[test]
fn encode_v2_swap_zero() {
    assert_eq!(encode_v2_swap(U256::ZERO, U256::ZERO, Address::ZERO).len(), 4 + 5 * 32);
}
#[test]
fn classify_transfer_boundary() {
    let v1 = classify_transfer(U256::from(10_000u64), U256::from(9_989u64), 10);
    assert!(matches!(v1, FotVerdict::FeeOnTransfer { tax_bps: 11 }), "got {v1:?}");
...
fn expected_amount_out_asymmetric() {
    let out = expected_amount_out(U256::from(1_000u64), U256::from(1_000u64), U256::from(1_000_000u64), 30);
...
fn discover_slot_multiple() {
    ...
    assert_eq!(discover_balance_slot(r, h, U256::from(111u64), 40), Some(U256::from(5u64)));
    assert_eq!(discover_balance_slot(r, h, U256::from(222u64), 40), Some(U256::from(10u64)));
...
async fn screen_transfer_inconclusive_zero_balance() {
    ...
    let cfg = FotConfig { max_tax_bps: 10, test_fraction_bps: 10, max_slot_probe: 40, gas_limit: 1_000_000 };
...
async fn screen_transfer_fot_zero_tolerance() {
    ...
    anvil.set_storage(FOT_ADDR, erc20_balance_key(POOL_ADDR, U256::ZERO), pb);
    let state = anvil.create_fork_state_warm(&[FOT_ADDR]).await;
    let cfg = FotConfig { max_tax_bps: 0, test_fraction_bps: 10, max_slot_probe: 40, gas_limit: 1_000_000 };
    let _v = screen_token_transfer(state, FOT_ADDR, POOL_ADDR, pb, &cfg);
}
...
async fn screen_transfer_one_wei() {
    ...
    anvil.set_storage(TOKEN_ADDR, erc20_balance_key(POOL_ADDR, U256::ZERO), pb);
    let state = anvil.create_fork_state_warm(&[TOKEN_ADDR]).await;
    let cfg = FotConfig { max_tax_bps: 10, test_fraction_bps: 1, max_slot_probe: 40, gas_limit: 1_000_000 };
    let _v = screen_token_transfer(state, TOKEN_ADDR, POOL_ADDR, pb, &cfg);
}
...
async fn screen_transfer_fot_high_tax() {
    ...
    anvil.set_storage(FOT_ADDR, erc20_balance_key(POOL_ADDR, U256::ZERO), pb);
    let state = anvil.create_fork_state_warm(&[FOT_ADDR]).await;
    let cfg = FotConfig { max_tax_bps: 100, test_fraction_bps: 10, max_slot_probe: 40, gas_limit: 1_000_000 };
    let _v = screen_token_transfer(state, FOT_ADDR, POOL_ADDR, pb, &cfg);
}
...
async fn screen_transfer_different_slot() {
    ...
    anvil.set_code(TOKEN_ADDR, SIMPLE_ERC20);
    anvil.set_storage(TOKEN_ADDR, erc20_balance_key(POOL_ADDR, U256::from(3u64)), pb);
    let state = anvil.create_fork_state_warm(&[TOKEN_ADDR]).await;
    let cfg = FotConfig { max_tax_bps: 10, test_fraction_bps: 10, max_slot_probe: 10, gas_limit: 1_000_000 };
    let _v = screen_token_transfer(state, TOKEN_ADDR, POOL_ADDR, pb, &cfg);
}
...
async fn screen_v2_round_trip_zero_reserves() {
    ...
    let state = anvil.create_fork_state_warm(&[TOKEN_ADDR, BASE_ADDR, PAIR_ADDR]).await;
    let cfg = FotConfig { max_tax_bps: 10, test_fraction_bps: 10, max_slot_probe: 40, gas_limit: 1_000_000 };
    let _v = screen_token_v2_round_trip(state, PAIR_ADDR, TOKEN_ADDR, BASE_ADDR, U256::ZERO, 30, &cfg, 300);
}
...
async fn screen_v2_round_trip_low_slot_probe() {
    ...
    anvil.set_storage(PAIR_ADDR, U256::ZERO, addr_to_u256(TOKEN_ADDR));
    anvil.set_storage(PAIR_ADDR, U256::from(1u64), addr_to_u256(BASE_ADDR));
    anvil.set_storage(PAIR_ADDR, U256::from(V2_RESERVES_SLOT), U256::from(100_000u64) | (U256::from(50_000u64) << 112));
    let state = anvil.create_fork_state_warm(&[TOKEN_ADDR, BASE_ADDR, PAIR_ADDR]).await;
    let cfg = FotConfig { max_tax_bps: 10, test_fraction_bps: 10, max_slot_probe: 1, gas_limit: 1_000_000 };
    let _v = screen_token_v2_round_trip(state, PAIR_ADDR, TOKEN_ADDR, BASE_ADDR, U256::ZERO, 30, &cfg, 300);
}
